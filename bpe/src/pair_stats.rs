...
/// Contagens de pares adjacentes. BPE clássico só precisa de `pair_counts`.
...
mod tests {
    ...
    #[test]
    fn from_words_counts_pairs() {
        // "abc" → [0,1,2] → pares (0,1) e (1,2)
        let words = vec![vec![0u32, 1, 2]];
        let stats = PairStats::from_words(&words);
        assert_eq!(*stats.pair_counts.get(&pack_pair(0, 1)).unwrap(), 1);
        assert_eq!(*stats.pair_counts.get(&pack_pair(1, 2)).unwrap(), 1);
    }
}
